...
#
SUBDIRS := RawViewer RecoViewer Utils TruthViewer #ADD_NEW_SUBDIR ... do not remove this comment from this line
...
#
clean:
	@for i in $(SUBDIRS); do ( echo "" && echo "Cleaning $$i..." && cd $(LARLITE_USERDEVDIR)/EventViewer/$$i && $(MAKE) clean && rm -f $(LARLITE_LIBDIR)/$$i.* ) || exit $$?; done
...
doxygen:
	...
	@mkdir -p $(LARLITE_USERDEVDIR)/RawViewer/doc/dOxygenMyProject
	@doxygen $(LARLITE_USERDEVDIR)/RawViewer/doc/doxygenMyProject.script
